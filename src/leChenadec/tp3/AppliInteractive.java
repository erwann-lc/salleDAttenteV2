...
//import salle.attente.SalleDAttentePrio;

public class AppliInteractive
{


	private static SalleDAttentePrio<ArticlePrio> file ;
	private static LinkedList<ArticlePrio> listeArticles ;
	...
	public static void main(String[] args) {
		try{
		    ...
		    file = new SalleDAttentePrio<ArticlePrio> (taille, prio) ;
		    listeArticles = new LinkedList<ArticlePrio>() ;
	...
	private static void modifierStock() throws IOException
	{
		...
		for (i = 0 ;
				i < listeArticles.size()
				   && ! (listeArticles.get(i)).getDesignation().equals(des) ;
				i++) ;
		...
		else
		{
			...
			ArticlePrio art = listeArticles.get(i) ;
			if (delta < 0)
			{
				art.retirer(-delta) ;
			}
